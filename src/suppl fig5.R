...
lapply(majorpop_list, function(x) {
    ...
    #   add_xy_position(x = 'class', dodge = 0.75)
    ...
    d.stats.class <- d.stats.class %>%
        mutate(
            xmin = xmin - 0.1875, # Shift left by half the dodge width
            xmax = xmax - 0.1875
        ) # Shift left by half the dodge width

    ggplot(d %>% filter(!is.na(NEC)), aes(x = class, y = freq, color = as.factor(NEC))) +
        ...
        stat_pvalue_manual(d.stats.class,
    ...
    ggexport(filename = "figures/suppl figure 5/5b.pdf", width = 10, height = 12)
...
lapply(majorpop_list, function(x) {
    ...
    ggplot(d %>% filter(!is.na(BPD)), aes(x = class, y = freq, color = as.factor(BPD))) +
        geom_boxplot(outlier.shape = NA, position = position_dodge(width = 0.75)) +
        geom_jitter(position = position_jitterdodge(jitter.width = 0.2), size = 1) +
    ...
    ggarrange(plotlist = ., ncol = 2, nrow = 3, common.legend = T) %>%
    ggexport(filename = "figures/suppl figure 5/5a.pdf", width = 10, height = 12)

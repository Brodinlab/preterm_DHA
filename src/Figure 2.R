...
lapply(majorpop_list, function(x) {
  tmp <- data.merge %>%
    filter(merge2 == x, GA_days - GA_days_at_birth < 2) %>%
    select(freq)
  yhigh <- max(tmp) * 1.1
  ylow <- min(tmp) * 0.9
  data.merge %>%
    filter(merge2 == x, GA_days - GA_days_at_birth < 2, class != "Term") %>%
...
# compare term and preterm
data.merge %>%
  filter(GA_days - GA_days_at_birth < 2, merge2 %in% majorpop_list) %>%
  mutate(term = if_else(class == "Term", "Term", "Preterm")) %>%
  group_by(merge2) %>%
  wilcox_test(freq ~ term, data = .) %>%
